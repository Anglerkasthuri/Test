...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.forms.form-field.form-field-filter')
                            </div>
                                    ...
                                    <tbody>
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                ...
                                                <td class="title">
                                                    {{ $record->title }}
                                                    {!! $record->is_required ? "<span class='text-danger'> * </span>" : "" !!}
                                                </td>
                                                <td>{{ $record->form_field_type->title ?? __("msg.na") }}</td>
                                                <td>{{ $record->sequence_number ?? __("msg.na") }}</td>
                                                <td>{{ $record->id }}</td>
    ...
    <x-common.modal wire:key="{{ now() }}"
        ...
        <x-slot name="modal_body">
            @include('livewire.admin.forms.form-field.form-field-create')
        </x-slot>
    </x-common.modal>
