...
<div>
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1>Program Subcategory</h1>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                        <li class="breadcrumb-item active">Program Subcategory</li>
                    </ol>
                </div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.program-subcategory.program-subcategory-filter')
                            </div>
                                ...
                                <table class="table table-head-fixed text-nowrap" wire:loading.class="loading">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>#</th>      
                                            <th wire:click="sortBy('program_category_id')"
                                                class="{{ $this->sortFieldData['program_category_id']['css_class'] ?? 'sortby' }}">
                                                Program Category</th>                                      
                                            <th wire:click="sortBy('title')"
                                                class="{{ $this->sortFieldData['title']['css_class'] ?? 'sortby' }}">
                                                Program Subcategory</th>
                                            <th>Short Name</th>
                                            <th wire:click="sortBy('sequence_number')"
                                            class="{{ $this->sortFieldData['sequence_number']['css_class'] ?? 'sortby' }}">
                                            Sequence</th>
                                    ...
                                    <tbody>
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                ...
                                                <td>{{ $record->program_category->title }}</td>
    ...
    <x-common.modal wire:key="{{ now() }}"
        ...
        <x-slot name="modal_title"> Program Subcategory </x-slot>
        <x-slot name="modal_size"> modal-lg </x-slot>
        <x-slot name="modal_body">
            @include('livewire.admin.masters.program-subcategory.program-subcategory-create')
        </x-slot>
    </x-common.modal>
