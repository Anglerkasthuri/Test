...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.master-group.master-group-filter')
                            </div>
                                ...
                                <table class="table table-head-fixed text-nowrap" wire:loading.class="loading">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>#</th>
                                            <th wire:click="sortBy('title')" class="{{ $this->sortFieldData['title']['css_class'] ?? 'sortby' }}">Master Group</th>
                                    ...
                                    <tbody>
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                ...
                                                <td>{{ $record->title }}</td>
                                                <td>{{ $record->master_category_count }}</td>
    ...
    <div class="modal_section" >
        ...
        :modal_section_id="'modalCreate'" >
            <x-slot name="modal_title">Master Group</x-slot>
            <x-slot name="modal_size">modal-lg</x-slot>
            <x-slot name="modal_header_action"></x-slot>
            <x-slot name="modal_body">
                @include('livewire.admin.masters.master-group.master-group-create')
            </x-slot>
        </x-common.modal>
    </div>
