...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.course.course-filter')
                            </div>
                                    ...
                                    <tbody>
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                ...
                                                <td>{{ $record->program_sub_category->title ?? __('msg.na')  }}</td>
                                                <td>{{ $record->program_level->title ?? __('msg.na')  }}</td>
                                                <td>{{ $record->course_type->title ?? __('msg.na')  }}</td>
                                                <td>{{ $record->course_category->title ?? __('msg.na')  }}</td>
    ...
    <x-common.modal wire:key="{{ now() }}"
        ...
        <x-slot name="modal_body">
            @include('livewire.admin.masters.course.course-create')
        </x-slot>
    </x-common.modal>
