...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.grade-category.grade-category-filter')
                            </div>
                                ...
                                <table class="table table-head-fixed text-nowrap" wire:loading.class="loading">
                                    <thead>
                                        <tr>
                                            ...
                                            <th wire:click="sortBy('internal_calculation_available')"
                                                class="{{ $this->sortFieldData['internal_calculation_available']['css_class'] ?? 'sortby' }}">
                                                Internal Available</th>
                                                <th wire:click="sortBy('external_calculation_available')"
                                                class="{{ $this->sortFieldData['external_calculation_available']['css_class'] ?? 'sortby' }}">
                                                External Available </th>
                                        ...
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                <td>
                                                    ...
                                                    <a href="{{ route('grade-category.pattern', ["grade_category_id" => $record->id ]) }}" data-toggle="popover" data-trigger="hover" data-content="Set Grade Pattern">
                                                ...
                                                <td>{{ $record->id }}</td>
                                                <td><span class="{{ config('settings.boolean_field_options')[$record->internal_calculation_available]['class'] }}"> {{ config('settings.boolean_field_options')[$record->internal_calculation_available]['display'] }} </span>
                                                    <td><span class="{{ config('settings.boolean_field_options')[$record->external_calculation_available]['class'] }}"> {{ config('settings.boolean_field_options')[$record->external_calculation_available]['display'] }} </span>
    ...
    <x-common.modal wire:key="{{ now() }}"
        ...
        <x-slot name="modal_body">
            @include('livewire.admin.masters.grade-category.grade-category-create')
        </x-slot>
    </x-common.modal>
