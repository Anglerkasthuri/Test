...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.master-category.master-category-filter')
                            </div>
                                ...
                                <table class="table table-head-fixed text-nowrap" wire:loading.class="loading">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>#</th>
                                            <th wire:click="sortBy('title')" class="{{ $this->sortFieldData['title']['css_class'] ?? 'sortby' }}">Master Category</th>
                                            <th wire:click="sortBy('code')" class="{{ $this->sortFieldData['code']['css_class'] ?? 'sortby' }}">Code</th>
                                            <th>Option</th>
                                            <th wire:click="sortBy('master_group_id')" class="{{ $this->sortFieldData['master_group_id']['css_class'] ?? 'sortby' }}">Master Group</th>
                                            <th>Show in Form</th>
                                            <th>Dependent</th>
                                        ...
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                <td>
                                                    ...
                                                    <a href="{{ route('master-option.category', ["master_category_id" => $record->id ]) }}" data-toggle="popover" data-trigger="hover" data-content="Add Options" data-placement="top">
                                                ...
                                                <td>{{ $record->title }}</td>
                                                <td>{{ $record->code }}</td>
                                                <td>{{ $record->master_option_count }}</td>
                                                <td>{{ $record->master_group->title }}</td>
                                                <td>
                                                    {!! 
                                                        !empty($record->show_in_form) 
                                                ...
                                                <td>
                                                    {!! 
                                                        !empty($record->is_dependent) 
                                                        ? $record->parent_category->title ?? null
                                                        : "<i class='text-danger fa fa-times'></i>" 
                                                    !!}
                                                </td>
    ...
    <div class="modal_section" >
        ...
        :modal_section_id="'modalCreate'" >
            <x-slot name="modal_title">Master Category</x-slot>
            <x-slot name="modal_size">modal-lg</x-slot>
            <x-slot name="modal_header_action"></x-slot>
            <x-slot name="modal_body">
                @include('livewire.admin.masters.master-category.master-category-create')
            </x-slot>
        </x-common.modal>
    </div>
