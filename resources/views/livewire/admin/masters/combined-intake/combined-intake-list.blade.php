...
<div>
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1>Combined Intake</h1>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                        <li class="breadcrumb-item active">Combined Intake</li>
                    </ol>
                </div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.masters.combined-intake.combined-intake-filter')
                            </div>
                                    ...
                                    <tbody>
                                        @forelse($records  as $index => $record)
                                            <tr class="{{ empty($record->active) ? ' table-disable ' : '' }}">
                                                ...
                                                <td>{{ $record->month->title ?? __('msg.na') }}</td>
                                                <td>{{ $record->year ?? __('msg.na')  }}</td>
                                                <td>{{ $record->id }}</td>
    ...
    <x-common.modal wire:key="{{ now() }}"
        ...
        <x-slot name="modal_body">
            @include('livewire.admin.masters.combined-intake.combined-intake-create')
        </x-slot>
    </x-common.modal>
