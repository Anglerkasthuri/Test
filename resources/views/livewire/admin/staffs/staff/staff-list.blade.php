...
<div>
                    ...
                    <div class="card">
                        <div class="card-body p-0">
                            <div wire:ignore class="filter-section collapse" id="filter_collapse">
                                @include('livewire.admin.staffs.staff.staff-filter')
                            </div>
                                ...
                                <table class="table table-head-fixed text-nowrap" wire:loading.class="loading">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                                ...
                                                <td>{{ $record->title }}</td>
                                                <td>{{ $record->date_of_joined_display ?? __("msg.na") }}</td>
                                                <td>{{ $record->employee_code ?? __("msg.na") }}</td>
                                                <td>{{ $record->staff_type->title ?? __("msg.na") }}</td>
                                                <td>{{ $record->work_type->title ?? __("msg.na") }}</td>
                                                <td>{{ $record->email }}</td>
                                                <td>{{ $record->id }}</td>
    ...
    <div class="modal_section" >
        ...
        :modal_section_id="'modalCreate'" >
            <x-slot name="modal_title">Staff</x-slot>
            <x-slot name="modal_size">modal-lg</x-slot>
            <x-slot name="modal_header_action"></x-slot>
            <x-slot name="modal_body">
                @include('livewire.admin.staffs.staff.staff-create')
            </x-slot>
        </x-common.modal>
    </div>
